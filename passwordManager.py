...
def createEntry():
    ...
    if user =="y":
        ...
        masterEntryList.append(newEntry)
        ...
        with open("data.csv", "r") as file:
                ...
                for row in reader:
                    if len(row) >= 4 and row[0] == username and row[1] == password:
                        ...
                        confirmCategory = input("Confirm your Category: ")
                        
                        if newusername != confirmusername and newPassword != confirmpassword and newfirst != confirmfirst and newlast != confirmlast and newCategory != confirmCategory: 
                            print("The creditionals do not match...")
                            return
                        first = row[2]
                        last = row[3]
                        updated_data.append([newusername, newPassword, newfirst, newlast, newCategory])
                    else:
...
def delete():
    global first,last
    notdelete = []
    delete = input("which account do you want to delete? ")
    file1 = open(f"{last}_{first}_entries.txt")
    file = file1.readlines()
    for eachline in file:
        if not eachline in file:
            file.append(delete)
            database()
        else:
            file.append(notdelete)
            database()
...
def login():
    ...
    if user == "register":
        ...
        with open("data.csv", "a", newline="") as file:
            writer = csv.writer(file)
            writer.writerow([username, password, first, last, category])
        print("Registration successful! Please log in.")
        login()
#if they choose login they login to there account they made
    elif user == "login":
        attempts = 0
        max_attempts = 3
        ...
        quit()
# how they navigate the database and edit and find things
def database():
    while True:
